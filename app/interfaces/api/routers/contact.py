...
async def submit_contact_form(
    ...
    # Triggers Celery task asynchronously and returns HTTP 202 Accepted
    await email_service.send_contact_message(
        name=message_data.name,
        email=message_data.email,
        message=message_data.message
    )
    return {"message": "Your message has been received and is being processed in the background"}
